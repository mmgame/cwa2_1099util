...
public class FileUtil {
	...
	public static List<String> readFileContentFromDirectory(String path, String endsWith) {
		...
		if (!dirFile.isDirectory()) {
			String content = readFileContent(dirFile);
		...
		for (File file : files) {
			...
			String sqlStr = readFileContent(file);
	 ...
	 */
	public static List<String> getRecursionAllFilePath(String workPath, String extensionName, String middleStr, List<String> paths) {
		File dir = new File(workPath);
		File file[] = dir.listFiles();
		for (int i = 0; i < file.length; i++) {
	 ...
	 */
	public static List<PathNameBean> getRecursionAllFileName(String workPath, String extensionName, List<PathNameBean> names) {
		File dir = new File(workPath);
		File file[] = dir.listFiles();
		for (int i = 0; i < file.length; i++) {
			if (file[i].isDirectory()) {
	 ...
	 */
	public static List<PathNameBean> getRecursionAllFileName(String workPath, String extensionName, List<PathNameBean> names, String folderName) {
		File dir = new File(workPath);
		File file[] = dir.listFiles();
		for (int i = 0; i < file.length; i++) {
